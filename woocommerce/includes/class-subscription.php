...
class WoocommerceIR_SMS_Subscription extends \WP_Widget {
	...
	public function displayForm( $product = '' ) {
        ...
        <form class="sms-notif-form" id="sms-notif-form-<?php echo esc_attr($id); ?>" method="post">
            <div style="display:none !important;width:0 !important;height:0 !important;">
                <img style="width:16px;display:inline;"
                     src="<?php echo SABAPAYAMAK_WOO_URL; ?>/assets/images/tick.png"/>
                <img style="width:16px;display:inline;"
                     src="<?php echo SABAPAYAMAK_WOO_URL; ?>/assets/images/false.png"/>
                <img style="width:16px;display:inline;"
                     src="<?php echo SABAPAYAMAK_WOO_URL; ?>/assets/images/ajax-loader.gif"/>
            </div>

            <div class="sms-notif-enable-p" id="sms-notif-enable-p-<?php echo  esc_attr($id); ?>">
                <label id="sms-notif-enable-label-<?php echo esc_attr($id); ?>" class="sms-notif-enable-label"
                       for="sms-notif-enable-<?php echo esc_attr($id); ?>">
                    <input type="checkbox" id="sms-notif-enable-<?php echo esc_attr($id); ?>" class="sms-notif-enable"
                           name="sms_notif_enable"
                           value="1">
                    <strong><?php echo esc_html(PWooSMS()->getValue( 'notif_title', $product_id )); ?></strong>
                </label>
            </div>
            ...
            <div class="sms-notif-content" id="sms-notif-content">
				<?php foreach ( $groups as $code => $text ) : ?>
                    ...
                    <label class="sms-notif-groups-label sms-notif-groups-label-<?php echo esc_attr($code); ?>"
                        ...
                        <input type="checkbox"
                               ...
                               class="sms-notif-groups" name="sms_notif_groups[]" value="<?php echo esc_attr($code); ?>"/>
                ...
                <div class="sms-notif-mobile-div">
                    <input type="text" id="sms-notif-mobile-<?php echo esc_attr($id); ?>" class="sms-notif-mobile"
                           name="sms_notif_mobile"
                           value="<?php echo esc_attr(get_user_meta( get_current_user_id(), PWooSMS()->buyerMobileMeta(), true )); ?>"
				...
				<?php if ( ! $can_be_subscribe ) : ?>
                    <p id="sms-notif-disabled-<?php echo esc_attr($id); ?>" class="sms-notif-disabled">
						<?php echo esc_html(PWooSMS()->getValue( 'notif_only_loggedin_text', $product_id )) ; ?>
                    </p>
				<?php else : ?>
                    <button id="sms-notif-submit-<?php echo esc_attr($id); ?>"
                ...
                <p id="sms-notif-result-p-<?php echo esc_attr($id); ?>" class="sms-notif-result-p">
                    <span id="sms-notif-result-<?php echo esc_attr($id); ?>" class="sms-notif-result"></span>
                </p>
            </div>
        </form>
		...
		if ( $id == 1 ) {
			...
			jQuery(document).ready(function($){
			    ...
			    $(document.body).on( "change", ".sms-notif-enable", function() {
				    ...
				    result.html( "<img style=\"width:16px;display:inline;\" src=\"' . SABAPAYAMAK_WOO_URL . '/assets/images/ajax-loader.gif\" />" );
			    	var sms_group = [];
	...
	public function updateSubscription() {
		...
		$error_image = '<img style="width:16px;display:inline;" src="' . SABAPAYAMAK_WOO_URL . '/assets/images/false.png">&nbsp;';
		...
		$mobile = isset( $_POST['sms_mobile'] ) ? PWooSMS()->modifyMobile( sanitize_text_field($_POST['sms_mobile']) ) : '';
		if ( empty( $mobile ) ) {
		...
		$groups = array_map('sanitize_text_field', ((array) $_POST['sms_group']));

		$success_image = '<img style="width:16px;display:inline;" src="' . SABAPAYAMAK_WOO_URL . '/assets/images/tick.png">&nbsp;';
...
add_action( 'widgets_init', function () {
	register_widget( 'SabaPayamak\WoocommerceIR_SMS_Subscription' );
} );