...
class WoocommerceIR_SMS_Orders {
	...
	public function __construct() {
		...
		if ( $this->enabled_buyers || $this->enable_super_admin_sms || $this->enable_product_admin_sms ) {
			...
			add_filter( 'woocommerce_form_field_pwoosms_multiselect', array(
				'SabaPayamak\WoocommerceIR_SMS_Helper',
	...
	public function checkoutScript() {
		...
		if ( PWooSMS()->Options( 'allow_buyer_select_status' ) && is_checkout() ) {
			...
			wp_register_script( 'pwoosms-frontend-js', SABAPAYAMAK_WOO_URL . '/assets/js/multi-select.js', array( 'jquery' ), SABAPAYAMAK_WOO_VERSION, true );
	...
	public function checkoutFieldsValidation() {
		...
		if ( ! empty( $_POST[ $mobile_meta ] ) ) {
			$_POST[ $mobile_meta ] = PWooSMS()->modifyMobile( sanitize_text_field($_POST[ $mobile_meta ]) );
		} else {
			$_POST[ $mobile_meta ] = '';
		}
		...
		if ( $buyer_selected && ! PWooSMS()->validateMobile( sanitize_text_field($_POST[ $mobile_meta ]) ) ) {
            ...
            <script type="text/javascript">
                jQuery( document ).ready( function ( $ ) {
                    $( "#order_status" ).change( function () {
                        $( "#pwoosms_textbox" ).html( "<img src=\"<?php echo SABAPAYAMAK_WOO_URL ?>/assets/images/ajax-loader.gif\" />" );
	...
	public function changeSmsTextCallback() {
		...
		if ( ! empty( $_POST['order_id'] ) ) {
			...
			echo '<textarea id="sms_order_text" name="sms_order_text" style="width:100%;height:120px;"> ' . esc_textarea($message) . ' </textarea>';
			...
			echo '<input type="checkbox" id="sms_order_send" class="sms_order_send" name="sms_order_send" value="true" style="margin-top:2px;width:20px; float:right" ' . esc_attr($sms_checked) . '/>
