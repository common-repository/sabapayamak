...
class WoocommerceIR_SMS_Helper {
	...
	public function multiSelectAdminField( $field ) {
		...
		foreach ( $field['options'] as $status_value => $status_name ) {
			echo '<option value="' . esc_attr( $status_value ) . '"' . selected( in_array( $status_value, $field['value'] ), true, false ) . '>' . esc_attr( $status_name ) . '</option>';
		}
	...
	public function nusoap() {
		if ( ! class_exists( 'nusoap_client' ) ) {
			require_once SABAPAYAMAK_WOO_INCLUDE_DIR . '/lib/nusoap.php';
		}
	}
}
