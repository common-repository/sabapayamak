...
class WoocommerceIR_SMS_Settings {
	...
	public function adminMenu() {
		add_menu_page('صباپیامک' , 'صباپیامک', 'manage_options', 'sabapayamak', array( $this, 'settingPage' ), 'none');
	}

	public function adminInit() {

		if ( ! empty( $_GET['tab'] ) && $_GET['tab'] == 'pwoosms_settings_page' ) {
			wp_redirect( admin_url( 'admin.php?page=sabapayamak' ) );
			exit();
		}
	...
	public function settingPage() {
		...
		$pattern_tags = array_column(Helpers::Get_tags(), 'tag');
        ?>
        <script>
            var ks_patternTags = <?php echo wp_kses(json_encode($pattern_tags), []); ?>;
        </script>

        <div id="ks_loading">
            <img src= "<?php echo SABAPAYAMAK_DIR_URL; ?>assets/loading.svg"></img>
        </div>
		<?php


		echo '<div class="wrap woocommerce ks_wrap">';
		$this->settings_api->show_navigation();
		KS_UI::notice_placeholder_snippet();
	...
	public function settingFields() {
		...
		$gateways_list = class_exists( 'SabaPayamak\WoocommerceIR_SMS_Gateways' ) ? WoocommerceIR_SMS_Gateways::get_sms_gateway() : array();
		asort( $gateways_list );
	...
	public function productAdminSettings( $settings ) {
		...
		$text = array(
			array(
				...
				'label' => '<h2>متن پیامک موجودی انبار</h2>',
				'desc'  => sprintf( 'با توجه به مشترک بودن متن پیامک های موجودی انبار بین مدیران کل و فروشندگان محصول، برای تنظیم متن این پیامک ها از %s استفاده کنید.', '<a href="' . admin_url( 'admin.php?page=sabapayamak&tab=super_admin#sms_super_admin_settings[admin_low_stock]' ) . '" target="_blank">این لینک</a>' ),
				'type'  => 'html',
			)
		);
